...
Each URL in the sitemap includes a `<lastmod>` date so search engines know when content was last updated. Documentation.AI derives this date from your page frontmatter timestamps, falling back to the previous deployment time if no timestamp is available. This means search engines see accurate last-modified dates per page rather than a single deployment-wide date.
