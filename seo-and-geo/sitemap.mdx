...
Each URL in the sitemap includes a `<lastmod>` date that reflects when the page was last updated. The system tracks per-page modification timestamps during deployment, so the `lastmod` value for each URL is accurate rather than using a single global timestamp for all pages. This helps search engines prioritize crawling pages that have actually changed.
