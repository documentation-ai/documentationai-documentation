...
When a reader hovers over a heading, a link icon appears. Clicking the heading copies its anchor URL to the clipboard so they can share a direct link to that section, and the icon briefly switches to a check mark after a successful copy. This works automatically with no configuration.
...
Headings created with `##` or `###` Markdown automatically get a hover link icon in the rendered page. Clicking any heading copies its anchor URL to the clipboard for easy sharing, with no extra syntax or configuration required.
