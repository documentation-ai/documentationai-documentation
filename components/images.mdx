...
## SVG and SVGClient components

Use `SVG` and `SVGClient` to fetch remote SVG files and render them inline so they inherit page CSS and styles. `SVG` renders server-side at build time, while `SVGClient` fetches and renders client-side in the browser.
...
### SVGClient with a custom className

```jsx
<SVGClient
  src="https://example.com/live-chart.svg"
  alt="Live data chart"
  className="my-chart"
/>
```
...
<ParamField body="src" param-type="string" required="true" deprecated="false">
  URL of the remote SVG file.
</ParamField>

<ParamField body="alt" param-type="string" required="false" deprecated="false">
  Accessibility text describing the SVG content.
</ParamField>

<ParamField body="className" param-type="string" required="false" deprecated="false">
  CSS class applied to the rendered SVG container.
</ParamField>

<ParamField body="fallback" param-type="ReactNode" required="false" deprecated="false">
  Content shown if the SVG fetch or parse fails.
</ParamField>

<Callout kind="alert" collapsed="false">
  Only remote SVGs served over HTTPS are supported. Inline scripts and event handlers are stripped during DOM parsing for security.
</Callout>
...
- **SVG handling:** Uploaded SVG files preserve interactive elements such as clickable links and hover states, unlike raster formats (PNG, JPEG) that go through compression and resizing.