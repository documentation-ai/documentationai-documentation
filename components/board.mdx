...
Card date props accept both kebab-case and camelCase in MDX.

<Callout kind="info">

Both `due-date` and `dueDate` are accepted, as are `created-at` and `createdAt`. The component prefers the camelCase value when both are present and falls back to the kebab-case value otherwise. Use whichever convention matches your style — the examples on this page use kebab-case.
...
Due date associated with the card. Use a date string such as `2025-02-01`. Both `due-date` and `dueDate` are accepted.
</ParamField>
...
Creation date for the card. Use a date string such as `2025-01-15`. Both `created-at` and `createdAt` are accepted.
</ParamField>
