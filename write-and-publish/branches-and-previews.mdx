...
- **Unsaved changes** — Edits you have made but not yet saved or published. These are synced to the server and visible to collaborators in real time, but are not committed to Git until you save or publish.
...
Unsaved changes behave differently from saved branch content. Unsaved edits are synced to the server and visible to collaborators in real time, but they are not committed to Git until you save, publish, merge, or clear them.

Use the **Changes** view to review both types of work: saved branch changes(if any) to check all the changes saved to that branch, while unsaved changes let you inspect unpublished edits before you save, merge, or publish.
...
<Steps>
  ...
  <Step title="Decide whether to carry unsaved changes" icon="edit" title-type="p">
    ...
    If you have unpublished edits, you can choose **Carry unsaved changes to new branch**.

    When you enable that option, Documentation.AI copies your unsaved edits to the new branch and removes them from the original branch after the switch succeeds.
...
<Callout kind="info" collapsed="false">
  ...
  Unsaved changes are synced to the server and visible to other collaborators in real time. They are not committed to Git until you save or publish through the branch workflow.
...
<Callout kind="alert" collapsed="false">
  ...
  If the Live branch has unsaved changes, merge is blocked until you publish those changes or clear them. This prevents unsaved edits on the Live branch from conflicting with the merge.
...
<ExpandableGroup>
  ...
  <Expandable title="Understand how unsaved changes work with collaboration" default-open="false">

    Unsaved edits are synced to the server and visible to other collaborators in real time, but they are not committed to Git until you save or publish.

    If you leave the editor, your unsaved changes are preserved on the server. Teammates connected to the same branch can see them, but they will not appear in a preview site or pull request until you save.
...
Branches give you a safer way to work than editing directly on the Live branch. You can create a branch from the editor top bar, choose whether to carry unsaved changes, review a preview site when available, and then merge to Live, publish from Live, or create a pull request.
